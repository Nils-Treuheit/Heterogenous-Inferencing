...
for model in models:
    ...
    for parts in partslist:
        ...
        plt.subplots_adjust(left=0.06,bottom=0.06,top=0.92,right=0.98)
        if SAVE_VIOLIN: 
            plt.savefig("violin_plots/async_batch_avg_runtime_"+model+".png")
            plt.savefig("violin_plots/async_batch_avg_runtime_"+model+".pdf")
        if LOG_STATS:
            log_file.write("\n")
            log_file.close()
...
for part in range(3):
    ...
    if SAVE_STATS: 
        ...
        plt.savefig("plots/async_batch_avg_runtime_"+fname[part]+"_stats.pdf")
