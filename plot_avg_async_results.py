...
for model in models:
    ...
    if len(data)>0:
        ...
        plt.ylabel('Runtime')
        plt.title(model)
        if SAVE_VIOLIN: plt.savefig("violin_plots/async_batch_avg_runtime_"+model+".png")
        if LOG_STATS:
            log_file.write("\n")
            log_file.close()
...
for part in range(3):
    ...
    fig.suptitle('AVG in batch of 64 Inferences on')
    if SAVE_STATS: plt.savefig("plots/async_batch_avg_runtime_"+fname[part]+"_stats.png")
